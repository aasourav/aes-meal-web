...
const Home = () => {
  ...
  const [mealData, setMealData]= useState<IMealRecord[]>()
  const [table,setTable]=useState<{
    date: string;
    mealCount: number;
    key: number;
}[]>()
  const onMealDataChange = async(filterType : keyof IMealHistory, value : number)=>{
    setMealHistory((prev)=> ({
      ...prev,
      [filterType]:value
    }))
  ...
  return (
    <MainContainer>
      <LeftContainer>
        ...
        {userDetails.pendingWeeklyMealPlan.length >0 ? <p>Pending weekly plan</p> : null}
        {userDetails.pendingWeeklyMealPlan.length >0 ? <Button onClick={onClickPendingWeeklyPlan} size="small" type="primary" danger>Delete</Button> : null}

        </div>
        {userDetails.pendingWeeklyMealPlan.length >0 ? 
          userDetails.pendingWeeklyMealPlan.map((data,index)=> (<Checkbox  disabled checked={data}>{weekOfDay[index]}</Checkbox>))
          : null
          }
      </LeftContainer>
      <RightContainer>
...
<Select
  ...
  {isUpdate ? <Button onClick={onWeeklyUpdateChange} size="small" type="primary">Update</Button>: null}
